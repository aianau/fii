...
int Tournament(int tsize, std::deque<Proposition>& population, unsigned int populationSize)
{
	...
	{
		rand_pos.at(i) = (Ardo::Numbers::Get_random<float>(0, 1)) * populationSize;
		if (rand_pos.at(i) >= populationSize)
			rand_pos.at(i) = populationSize - 1;
	}
	for (int i = 0; i < tsize; i++)
...
bool Tournament_Selection(std::deque<Proposition>& population, std::deque<std::pair<uint32_t, uint32_t>>& clauses_negative, Proposition& proposition_aux)
{
	...
	for (int i = 0; i < populationSize; i++)
	{
		int kk = Tournament(13, population, populationSize);
...
bool Tournament_Selection_GPU(std::deque<Proposition>& population, std::deque<std::pair<uint32_t, uint32_t>>& clauses_negative, Proposition& proposition_aux)
{
	int max_fitness = 0;
	int max_index;
	...
	printf("%d ", max_fitness);
	fprintf(fp_out_2, "%d\n", max_fitness);
	fflush(fp_out_2);
	if (max_fitness == 19054)
	{
		population.at(max_index).Evaluate(clauses_negative);
	}

	for (int i = 0; i < populationSize; i++)
	{
		int kk = Tournament(5, population, populationSize);
		population_2.at(i).Subject_cpy(population.at(kk));
	}
	for (int i = 0; i < populationSize; i++)
	{
...
bool catastrophe(std::deque<Proposition>& population, int populationSize, int cat_size)
{
...
bool Run(FILE *fp_out, std::string input_file)
{
	...
	for (int i = 0; i < Ardo::Numbers::Constants::ITERATIONS; ++i)
	{
		...
		Tournament_Selection_GPU(population, clauses_negative, proposition_aux);
		printf("%d ", i);	
		auto end = std::chrono::system_clock::now();
		std::chrono::duration<double> elapsed_seconds = end - start;
		printf("elapsed time: %f\n", elapsed_seconds.count());
		if (i % 200 == 199)
			catastrophe(population, populationSize, POPSIZE/2);
...
bool Main(std::string input_file)
{
	...
	std::string output_file_2(input_file);
	output_file_2.append(".iter");

	fp_out = fopen(output_file.c_str(), "w+");
	fp_out_2 = fopen(output_file_2.c_str(), "w+");


	for (int i = 0; i < 30 /*Ardo::Numbers::Constants::STATISTIC_NUMBER_TIMES_MIN*/; ++i)
	{
		auto start = std::chrono::system_clock::now();
		printf("iteration %d\n", i);
...
int main()
{
	...
	Main(".\\input\\frb35-17-cnf\\frb35-17-1.cnf");
