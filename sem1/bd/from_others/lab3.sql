...
/*2.Pentru fiecare student afi?a?i numele, data de nastere si numãrul de luni între data curentã ?i data na?terii.*/
select nr_matricol,nume,prenume,data_nastere,round(months_between(sysdate,data_nastere)) as diferenta
from studenti
order by 5 desc;

/*3.Afi?a?i ziua din sãptãmânã în care s-a nãscut fiecare student.*/
select nr_matricol,nume,prenume,nvl(to_char(data_nastere,'day'),'data nec') as zi_nastere
from studenti
order by 4;

/*4.Utilizând functia de concatenare, ob?ine?i pentru fiecare student textul 'Studentul <prenume> este in grupa <grupa>'.*/
...
/*6.Pentru profesorii al cãror nume începe cu B, afi?a?i numele cu prima litera micã si restul mari, precum ?i lungimea (nr. de caractere a) numelui.*/
...
/*8.Ordona?i studen?ii care nu iau bursã în func?ie de luna cand au fost nãscu?i; se va afi?a doar numele, prenumele ?i luna corespunzãtoare datei de na?tere.*/
...
/*10.Afiºaþi numele tuturor studen?ilor înlocuind apariþia literei i cu a si apariþia literei a cu i.*/
...
/*12.Presupunând cã în urmãtoarea lunã bursa de 450 RON se mãre?te cu 10%, cea de 350 RON cu 15% ?i cea de 250 RON cu 20%, 
afi?a?i pentru fiecare student numele acestuia, data corespunzãtoare primei zile din luna urmatoare ?i valoarea bursei pe care o va încasa luna urmãtoare.
