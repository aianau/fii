...
/*2.Scrie?i ?i executa?i o interogare pentru a afi?a în mod unic valorile burselor.*/
...
/*3.Da?i fiecãrei coloane din clauza select de la interogarea 1 un alias. Executa?i înterogarea.*/
...
/*4.Afi?a?i numele concatenat cu prenumele urmat de virgulã ?i anul de studiu. Ordona?i crescãtor dupã anul de studiu. Denumi?i coloana “Studen?i pe ani de studiu”.*/
select nume||' '||prenume||', '||an as "Studenti pe ani de studiu"
from studenti
order by an;

/*5.Afi?a?i numele, prenumele ?i data de nastere a studen?ilor nãscu?i între 1 ianuarie 1995 si 10 iunie 1997. Ordona?i descendent dupã anul de studiu.*/
...
/*6.Afi?a?i numele ?i prenumele precum ?i anii de studiu pentru to?i studen?ii nãscu?i în 1995.*/
select nume,prenume,an
from studenti
where extract(year from data_nastere)=1995
order by 1;

/*7.Afi?a?i studen?ii (toate informa?iile pentru ace?tia) care nu iau bursã.*/
...
/*8.Afi?a?i studen?ii (nume ?i prenume) care iau bursã ?i sunt în anii 2 ?i 3 de studiu. Ordona?i alfabetic ascendent dupã nume ?i descendent dupã prenume.*/
...
/*9.Afi?a?i studen?ii care iau bursã, precum ?i valoarea bursei dacã aceasta ar fi mãritã cu 15%.*/
select nume,prenume,bursa*1.15 as bursa_marita
from studenti
where bursa is not null and bursa<>0
order by 1;

/*10.Afi?a?i studen?ii al cãror nume începe cu litera P ?i sunt în anul 1 de studiu.*/
select nume,prenume
from studenti
where an=1 and nume like 'P%'
order by 1;

/*11.Afi?a?i toate informa?iile despre studen?ii care au douã apari?ii ale literei “a” în prenume.*/
select * from studenti
...
/*12.Afi?a?i toate informa?iile despre studen?ii al cãror prenume este “Alexandru”, “Ioana” sau “Marius”.*/
select * from studenti
...
/*14.Afiºaþi numele ºi prenumele profesorilor a cãror prenume se terminã cu litera "n" (întrebare capcanã).*/
select nume,prenume
from profesori
