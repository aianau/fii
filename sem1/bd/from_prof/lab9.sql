...
/*5.Afisati valoarea bursei/cap de student (se considerã cã studentii care nu sunt bursieri primesc 0 RON); 
altfel spus: cât se cheltuieste în medie pentru un student?*/
...
/*6.Afisati numãrul de note de fiecare fel (câte note de 10, câte de 9,etc.). Ordonati descrescãtor dupã valoarea notei.*/
...
/*7.Afisati numarul de note pus în fiecare zi a sãptãmânii. Ordonati descrescãtor dupã numarul de note.*/
...
/*8.Afisati numãrul de note pus în fiecare zi a sãptãmânii. Ordonati crescãtor dupã ziua saptamanii: Sunday, Monday, etc.*/
...
/*9.Afisati pentru fiecare student care are mãcar o notã, numele si media notelor sale. Ordonati descrescãtor dupã valoarea mediei*/
...
/*10.Modificati interogarea 9 pentru a afisa si studentii fãrã nici o notã. Media acestora va fi null.*/
...
/*11.Modificati interogarea 10 pentru a afisa pentru elevii fãrã nici o notã media 0.*/
...
/*13.Afisati numele, cea mai mare notã, cea mai micã notã si media doar pentru acei studenti care au cea mai micã notã mai mare sau egalã cu 7.*/
...
/*14.Afisati numele si mediile studentilor care au cel putin un numãr de 4 note puse în catalog.*/
...
/*16.Afisati cea mai mare medie obtinutã de vreun student.*/
...
/*17.Pentru fiecare disciplinã de studiu afi?ati titlul acesteia, cea mai micã ?i cea mai mare notã pusã.*/
